...
const ContactUs = ({id,darkMode}) => {
  ...
  return (
    ...
    id={id}
      ref={sectionRef}
      className={`min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 py-20 px-6 lg:px-20 ${darkMode ? 'bg-black' : 'bg-white'}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.8 }}
    >
      <div 
      className={`max-w-[1200px] mx-auto px-4 md:px-6 pt-12 md:pt-24 pb-8 md:pb-12 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}      >
        <motion.h1
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          {/* 3D Model Section */}
          <div className="relative h-[500px] rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-br from-indigo-100 to-purple-100">
            <Canvas>
